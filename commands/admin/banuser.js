...
export default {
  ...
  category: "admin",
  description: "Bloqueia ou desbloqueia usuários do bot",

  async run({ sock, msg, from, args = [], esDono, sender, settings, commandName }) {
    if (!esDono) {
    ...
    if (normalizedCommand === "banlist") {
      const lines = Object.entries(users).map(
        ([number, info]) =>
          `- ${formatUserNumber(number)} | ${String(info.reason || "Sem motivo")} | ${new Date(
    ...
    if (normalizedCommand === "unbanuser") {
      ...
      if (!target.number) {
        return sock.sendMessage(
          from,
          {
            text: `Usa: *${prefix}unbanuser 519xxxxxxx* o responde al usuário.`,
            ...global.channelInfo,
          },
          getQuoted(msg)
        );
      ...
      return sock.sendMessage(
        from,
        {
          text: `Usuário desbloqueado: *${formatUserNumber(target.number)}*`,
          ...global.channelInfo,
        },
        getQuoted(msg)
      );
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `o responde al usuário para bloquearlo.`,
          ...global.channelInfo,
        },
        getQuoted(msg)
      );
    ...
    return sock.sendMessage(
      ...
      {
        text:
          `Usuário bloqueado: *${formatUserNumber(target.number)}*\n` +
          `Motivo: *${reason}*`,
        ...global.channelInfo,
      },
      getQuoted(msg)
  ...
  async onMessage({ sock, msg, from, sender, esDono, settings }) {
    if (esDono) return false;
