...
export default {
  ...
  command: ["vip"],
  category: "admin",
  description: "Administra usuários VIP con tempo, usos y panel mejorado",
  donoOnly: true,
  ...
  run: async ({ sock, msg, from, args = [], settings }) => {
    ...
    if (action === "check") {
      ...
      if (!number || !info) {
        return sock.sendMessage(from, { text: "Ese número no es VIP.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "add" || action === "extend") {
      ...
      return sock.sendMessage(
        ...
        {
          text:
            `VIP ${action === "add" ? "adicionado" : "extendido"}.\n` +
            `Número: *${number}*\n` +
            `Usos: *${next.usesLeft}*\n` +
    ...
    if (action === "del" || action === "remove" || action === "rm") {
      ...
      delete data.users[number];
      saveVip(data);
      return sock.sendMessage(from, { text: `VIP removido: *${number || "sen número"}*`, ...global.channelInfo }, { quoted: msg });
    }

    return sock.sendMessage(
      from,
      {
        text: `Subcomando inválido. Usa ${prefix}vip`,
        ...global.channelInfo,
      },
      { quoted: msg }
    );
