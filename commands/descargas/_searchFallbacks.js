...
function mapLegacyTikTokItem(item = {}) {
  ...
  return {
    ...
    author: cleanText(authorData.unique_id || item?.author || "usuário").toLowerCase(),
...
function mapTikwmItem(item = {}) {
  ...
  return {
    ...
    author: cleanText(authorData.unique_id || "usuário").toLowerCase(),
...
async function requestLegacyTikTokSearch(query) {
  ...
  if (!data?.status || !Array.isArray(data?.result) || !data.result.length) {
    throw new Erro("legacy tiktok search unavailable");
  }
...
async function requestTikwmSearch(query, limit = 10) {
  ...
  if (!items.length) {
    throw new Erro("tikwm search unavailable");
  }
...
async function requestBingImageSearch(query, limit = 10) {
  ...
  if (!images.length) {
    throw new Erro("bing image search unavailable");
  }
