...
const STORE_FILE = path.join(process.cwd(), "database", "sorteios.json");
...
function activeSorteio(groupId = "") {
...
function participantsText(participants = []) {
  if (!participants.length) return "Sen participantes todavia.";
  return participants
    .slice(0, 80)
...
function createSorteio(groupId, payload = {}, sender = "") {
  const state = ensureGroupState(groupId);
...
function closeSorteio(groupId, reason = "auto") {
  const state = ensureGroupState(groupId);
  const active = activeSorteio(groupId);
...
async function announceClose(sock, groupId, closed) {
  ...
  const winnerLine = winnerMention
    ? `🥇 Ganhador: @${winnerMention.split("@")[0]}`
    : "⚠️ No hubo participantes, sen ganhador.";
...
async function sweepExpiredSorteios(sock) {
  const now = Date.now();
  ...
  for (const [groupId, state] of Object.entries(groups)) {
    ...
    const closed = closeSorteio(groupId, "auto");
...
export default {
  name: "sorteio",
  command: ["sorteio", "giveaway", "rifa"],
  category: "grupo",
  description: "Sorteios en grupo con inscripcion y cierre automatico",
  groupOnly: true,

  run: async ({ sock, msg, from, sender, args = [], settings, esAdmin = false, esDono = false, commandName = "" }) => {
    await sweepExpiredSorteios(sock);
    ...
    if (!action || ["menu", "help", "ajuda"].includes(action)) {
      const active = activeSorteio(from);
      const statusText = active
        ? `Sorteio ativo: *${active.prize}*\nTempo restante: *${formatDuration(Math.max(0, Number(active.endAtMs || 0) - Date.now()))}*`
        : "No hay sorteio ativo en este grupo.";
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `- ${prefix}sorteio sair\n` +
            `- ${prefix}sorteio status\n` +
            `- ${prefix}sorteio fechar (admin)\n` +
            `- ${prefix}sorteio cancelar (admin)`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "criar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede criar sorteios.", ...global.channelInfo }, { quoted: msg });
      }
      if (activeSorteio(from)) {
        return sock.sendMessage(from, { text: "Ya hay un sorteio ativo en este grupo.", ...global.channelInfo }, { quoted: msg });
      }
      ...
      if (!input) {
        return sock.sendMessage(
          from,
          {
            text:
              `Uso:\n` +
              `- ${prefix}sorteio criar 10m | Nitro Discord\n` +
              `- ${prefix}sorteio criar Premio sorpresa | 30m`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
      ...
      const created = createSorteio(from, input, sender);
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `┃ Unete con: *${prefix}sorteio unirme*\n` +
            `╰━━━━━━━━━━━━━━━━━━━━━━⬣`,
          ...global.channelInfo,
        },
        { quoted: msg }
    ...
    const active = activeSorteio(from);
    if (!active) {
      return sock.sendMessage(from, { text: "No hay sorteio ativo en este grupo.", ...global.channelInfo }, { quoted: msg });
    }

    if (action === "unirme" || action === "join" || action === "entrar") {
      const participant = cleanText(sender);
      if (!participant) {
        return sock.sendMessage(from, { text: "No pude detectar tu usuário.", ...global.channelInfo }, { quoted: msg });
      }
      if (!Array.isArray(active.participants)) active.participants = [];
      if (active.participants.includes(participant)) {
        return sock.sendMessage(from, { text: "Ya estas inscrito en este sorteio.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "sair" || action === "leave") {
      ...
      if (active.participants.length === before) {
        return sock.sendMessage(from, { text: "No estabas inscrito en este sorteio.", ...global.channelInfo }, { quoted: msg });
      }
      store.scheduleSave();
      return sock.sendMessage(
        from,
        {
          text: `Pronto, saliste del sorteio.\nParticipantes: *${active.participants.length}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "fechar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede fechar sorteios.", ...global.channelInfo }, { quoted: msg });
      }
      const closed = closeSorteio(from, "manual");
    ...
    if (action === "cancelar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede cancelar sorteios.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    return sock.sendMessage(
      from,
      { text: `Accion invalida. Usa *${prefix}sorteio*`, ...global.channelInfo },
      { quoted: msg }
    );
  },
