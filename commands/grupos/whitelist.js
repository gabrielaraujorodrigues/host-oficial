...
function formatList(values = []) {
  if (!values.length) return "Sen usuários en whitelist.";
...
export default {
  ...
  run: async ({ sock, from, msg, m, args = [], settings, groupMetadata }) => {
    ...
    if (!action || ["list", "lista", "status", "status"].includes(action)) {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `${formatList(list)}\n\n` +
            `Uso:\n` +
            `• ${prefix}whitelist add @usuário\n` +
            `• ${prefix}whitelist del @usuário\n` +
    ...
    if (["clear", "limpar", "reset"].includes(action)) {
    ...
    if (["add", "agregar", "set"].includes(action)) {
      ...
      if (!target) {
        return sock.sendMessage(
          from,
          {
            text:
              `⚠️ Indica usuário.\n` +
              `Ejemplo: ${prefix}whitelist add @usuário\n` +
              `Também puedes responder un mensagem.`,
            ...global.channelInfo,
          },
          quoted
        );
      ...
      if (!result.ok) {
        return sock.sendMessage(
          from,
          {
            text: "❌ No pude agregar ese usuário a whitelist.",
            ...global.channelInfo,
          },
          quoted
        );
      ...
      return sock.sendMessage(
        ...
        {
          text:
            `${result.existed ? "ℹ️ Ya estaba" : "✅ Adicionado"} en whitelist: ` +
      ...
      if (!target) {
        return sock.sendMessage(
          from,
          {
            text:
              `⚠️ Indica usuário.\n` +
              `Ejemplo: ${prefix}whitelist del @usuário\n` +
              `Também puedes responder un mensagem.`,
            ...global.channelInfo,
          },
          quoted
