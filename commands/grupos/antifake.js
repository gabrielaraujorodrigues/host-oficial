...
export default {
  ...
  groupOnly: true,
  adminOnly: true,
  description: "Bloqueia números fora dos prefixos permitidos",
  ...
  async run({ sock, msg, from, args = [] }) {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Prefixos permitidos: *${config.prefixes.join(", ")}*\n\n` +
            `.antifake on\n` +
            `.antifake off\n` +
            `.antifake add 52\n` +
            `.antifake remove 52\n` +
    ...
    if (action === "on") {
      ...
      return sock.sendMessage(
        from,
        { text: "Antifake ativado.", ...global.channelInfo },
        quoted
      );
    }
    ...
    if (action === "off") {
      ...
      return sock.sendMessage(
        from,
        { text: "Antifake desativado.", ...global.channelInfo },
        quoted
      );
    }
    ...
    if (action === "add") {
      ...
      return sock.sendMessage(
        from,
        { text: `Prefixo adicionado: ${value}`, ...global.channelInfo },
        quoted
      );
    }
    ...
    if (action === "remove" || action === "del") {
      ...
      return sock.sendMessage(
        from,
        { text: `Prefixo removido: ${value}`, ...global.channelInfo },
        quoted
      );
    }
    ...
    return sock.sendMessage(
      from,
      { text: "Opção invalida.", ...global.channelInfo },
      quoted
    );
  },
  ...
  async onGroupUpdate({ sock, update, settings }) {
    ...
    for (const participant of update.participants || []) {
      ...
      if (!number || isAllowed(number, config) || donoNumbers.includes(number) || number === botNumber) {
        continue;
      }
      ...
      await sock.sendMessage(update.id, {
        text:
          ...
          `No coincide con los prefixos permitidos del grupo.`,
