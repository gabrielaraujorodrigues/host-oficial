...
const STORE_FILE = path.join(process.cwd(), "database", "votaçãoes.json");
...
function createPoll(groupId, payload = {}, sender = "") {
  ...
  state.active = {
    id,
    question: cleanText(payload.question) || "Votação",
...
function pollResultText(poll) {
  ...
  let winnerLine = "Sen votos todavia.";
...
export default {
  name: "votação",
  command: ["votação", "encuesta", "votar"],
  category: "grupo",
  description: "Votaçãoes de grupo con cierre automatico",
  groupOnly: true,
  ...
  run: async ({ sock, msg, from, sender, args = [], settings, esAdmin = false, esDono = false, commandName = "" }) => {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `- ${prefix}votar 1\n` +
            `- ${prefix}votação status\n` +
            `- ${prefix}votação fechar (admin)\n` +
            `- ${prefix}votação cancelar (admin)`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "criar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede criar votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
      if (activePoll(from)) {
        return sock.sendMessage(from, { text: "Ya hay una votação activa en este grupo.", ...global.channelInfo }, { quoted: msg });
      }
      ...
      if (!input) {
        return sock.sendMessage(
          from,
          {
            text:
              `Uso:\n` +
              `- ${prefix}votação criar 10m | Elegimos hora | 8PM | 9PM\n` +
              `- ${prefix}votação criar Reunion | 30m | Hoy | Manana`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
    ...
    if (!poll) {
      return sock.sendMessage(from, { text: "No hay votação activa en este grupo.", ...global.channelInfo }, { quoted: msg });
    }

    if (action === "votar" || action === "vote") {
      const optionRaw = cleanText(payload.join(" "));
      if (!optionRaw) {
        return sock.sendMessage(from, { text: `Usa: *${prefix}votar <número>*`, ...global.channelInfo }, { quoted: msg });
      }
      const index = resolveOptionIndex(poll, optionRaw);
      if (index < 0) {
        return sock.sendMessage(from, { text: "Opção invalida para esta votação.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "status" || action === "resultados" || action === "resumen") {
    ...
    if (action === "fechar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede fechar votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "cancelar") {
      if (!(esAdmin || esDono)) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede cancelar votaçãoes.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    return sock.sendMessage(
      from,
      { text: `Accion invalida. Usa *${prefix}votação*`, ...global.channelInfo },
      { quoted: msg }
    );
  },
