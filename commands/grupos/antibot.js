...
function looksLikeBotName(name = "") {
  ...
  return /(bot|assestant|robot|autoresponder|auto|ia|ai)/i.test(value);
}
...
export default {
  ...
  category: "grupo",
  description: "Detecta clones/bots por comportamiento de comandos y nome",
  groupOnly: true,
  adminOnly: true,
  ...
  run: async ({ sock, from, msg, args = [] }) => {
    ...
    if (!action) {
      return sock.sendMessage(
        from,
        {
          text:
            `🤖 *ANTIBOT CLONE*\n` +
            `Status: *${enabledGroups.has(from) ? "ON ✅" : "OFF ❌"}*\n\n` +
            `Uso:\n` +
            `• .antibot on\n` +
            `• .antibot off`,
          ...global.channelInfo,
    ...
    if (action === "on") {
      ...
      return sock.sendMessage(
        from,
        { text: "✅ Antibot clone ativado en este grupo.", ...global.channelInfo },
        quoted
      );
    }
    ...
    if (action === "off") {
      ...
      return sock.sendMessage(
        from,
        { text: "✅ Antibot clone desativado en este grupo.", ...global.channelInfo },
        quoted
      );
    }
    ...
    return sock.sendMessage(
      from,
      { text: "❌ Opção invalida. Usa: on / off", ...global.channelInfo },
      quoted
    );
  },

  onMessage: async ({ sock, msg, from, esGrupo, esAdmin, esDono, groupMetadata, text, settings, comandos }) => {
    if (!esGrupo) return;
