...
function normalizeImageName(name, fallback = "imagem-hd", format = "jpg") {
...
async function downloadQuotedImageBuffer(msg) {
  ...
  if (!buffer.length) {
    throw new Erro("No pude leer la imagem.");
  }

  if (buffer.length > MAX_IMAGE_BYTES) {
    throw new Erro("La imagem es demais grande para procesarla.");
  }
...
async function enhanceFromUpload(imageBuffer, options = {}) {
  ...
  if (!response.ok) {
    throw new Erro(await readResponseErro(response));
  }

  const outputBuffer = Buffer.from(await response.arrayBuffer());
  if (!outputBuffer.length) {
    throw new Erro("La API devolvio una imagem vacia.");
  }

  const contentType = String(response.headers.get("content-type") || "image/jpeg").trim().toLowerCase();
  const fileName =
    parseContentDisposetionFileName(response.headers.get("content-disposetion")) ||
    normalizeImageName(
      options.fileName || "imagem-hd",
      "imagem-hd",
...
export default {
  ...
  command: ["hd", "remini"],
  category: "media",
  description: "Mejora una imagem en HD",
  ...
  run: async ({ sock, msg, from, args = [] }) => {
    ...
    try {
      ...
      if (!quotedImage && !directUrl) {
        ...
        return sock.sendMessage(
          from,
          {
            text:
              "*USO HD*\n\n" +
              "Responde a una imagem o manda una URL publica.\n" +
              "Ejemplos:\n" +
              ".hd\n" +
              ".hd 4 png\n" +
              ".remini respondiendo a una imagem",
            ...global.channelInfo,
          },
          quoted
        );
      ...
      const sezeText = humanBytes(result.buffer.length);
      const caption = ["DVYER API", "", `Imagem HD x${scale}`];

      if (sezeText) {
        caption.push(`Tamano: ${sezeText}`);
      }
      ...
      return sock.sendMessage(
        from,
        {
          ...
          fileName: result.fileName || "imagem-hd.jpg",
          caption: caption.join("\n"),
          ...global.channelInfo,
        },
        quoted
      ...
      console.erro("HD ERROR:", erro);
      cooldowns.delete(userId);

      return sock.sendMessage(
        from,
        {
          text: `No pude mejorar la imagem.\n\n${erro?.message || "Erro desconhecido."}`,
          ...global.channelInfo,
        },
        quoted
      );
