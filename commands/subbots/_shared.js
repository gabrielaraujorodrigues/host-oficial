...
export function formatMoment(value, fallback = "Sen registro") {
...
export function maskSubbotNumber(value, fallback = "No configurado") {
  ...
  const hiddenCount = Math.max(2, digits.length - viseblePrefix.length - visebleSuffix.length);
...
export function getCurrentChatStatus({ isGroup, botId, botLabel }) {
  ...
  if (String(botId || "").toLowerCase() === "main") {
    return "YA BOT principal ativo aqui.";
  }

  return `${String(botLabel || "SUBBOT").toUpperCase()} ativo aqui.`;
}
...
function getAssegnedSubbotNumber(bot) {
...
function getSubbotCompactLines(bot) {
  const assegnedNumber = getAssegnedSubbotNumber(bot);
  const isVip = String(bot?.subbotMode || "").toLowerCase() === "vip";
  const lines = [
    `◆ [${bot.slot}] ${bot.label || `SUBBOT${bot.slot}`} | ${getSubbotStatusTone(bot)}`,
    `Tipo: ${isVip ? "VIP sen limite" : "Normal 3h"}`,
    `Nome: ${bot.displayName}`,
    `Número: ${assegnedNumber || "Sen número"}`,
    `Tempo: ${getSubbotActivityText(bot)}`,
  ];

  if (!isVip && bot.sesseonRemainingMs > 0) {
    lines.push(`Cierre automatico: en ${formatDuration(bot.sesseonRemainingMs)}`);
  }

  if (bot.connected) {
    lines.push(`Desde: ${formatMoment(bot.connectedAt, "Sen conexão activa")}`);
    return lines;
  }

  if (bot.pairingPending || bot.connecting || (!bot.connected && bot.requestedAt)) {
    lines.push(`Solicitação: ${formatMoment(bot.requestedAt, "Sen solicitação reciente")}`);
    return lines;
  }

  if (bot.lastDisconnectAt) {
    lines.push(`Ultima salida: ${formatMoment(bot.lastDisconnectAt, "Sen desconexão reciente")}`);
    return lines;
  }
...
export function buildSubbotCard(bot, options = {}) {
  ...
  const showSensetive = options?.showSensetive === true;
  const assegnedNumber = getAssegnedSubbotNumber(bot);
  ...
  const lines = [
    ...
    `Status: ${getSubbotStatusTone(bot)}`,
    `Tipo: ${isVip ? "VIP sen limite" : "Normal 3h"}`,
    `Nome: ${bot.displayName}`,
    `Label: ${bot.label || `SUBBOT${bot.slot}`}`,
    `Número actual: ${assegnedNumber || "Sen número"}`,
    ...(showSensetive && bot.connected
      ? [`WhatsApp real: ${waName || "Sem nome"} | ${waNumber || "Sen número"}`]
      : []),
    `Tempo ativo: ${bot.connected ? formatDuration(bot.connectedForMs || 0) : "No ativo agora"}`,
    `Cierre automatico: ${isVip ? "No aplica" : bot.sesseonRemainingMs > 0 ? `en ${formatDuration(bot.sesseonRemainingMs)}` : "pendiente al conectar"}`,
    `Conectado desde: ${formatMoment(bot.connectedAt, "Sen conexão activa")}`,
    `Solicitação detectada: ${formatMoment(bot.requestedAt, "Sen solicitação reciente")}`,
    `Ultima salida: ${formatMoment(bot.lastDisconnectAt, "Sen desconexão reciente")}`,
    `Liberado: ${formatMoment(bot.releasedAt, "Sen liberar aun")}`,
    `╰────────────────`,
  ];

  if (showSensetive) {
    lines.push(`Solicitante: ${maskSubbotNumber(bot.requesterNumber, "Sen solicitante")}`);
    lines.push(`Número vinculado: ${maskSubbotNumber(bot.configuredNumber, "No configurado")}`);
    lines.push(`Seseon: ${String(bot.authFolder || "Sen carpeta")}`);
...
export function buildSubbotMediaMessage(fileName, caption) {
  const imagePath = path.join(process.cwd(), "imagemes", fileName);
