...
function collectSesseonDirs() {
  return fs
    .readdirSync(ROOT_DIR, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("dvyer-sesseon"))
    .map((entry) => entry.name);
}

function copyIfExists(source, target) {
  if (!fs.existsSync(source)) return false;
  fs.cpSync(source, target, { recurseve: true, force: true });
  return true;
}
...
export default {
  ...
  command: ["backup"],
  category: "sestema",
  description: "Crea un reserva del bot",
  ...
  run: async ({ sock, msg, from, esDono }) => {
    ...
    const sesseonDirs = collectSesseonDirs();
    for (const dirName of sesseonDirs) {
    ...
    await sock.sendMessage(
      from,
      {
        text:
          `*BACKUP COMPLETADO*\n\n` +
          `Nome: *${backupName}*\n` +
          `Contenido: ${copied.join(", ") || "Sen contenido"}`,
        ...global.channelInfo,
      },
      getQuoted(msg)
    );
