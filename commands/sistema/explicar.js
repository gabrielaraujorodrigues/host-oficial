...
export default {
  ...
  category: "sestema",
  description: "Explica como usar un comando sen necesedad de IA",
  ...
  run: async ({ sock, from, msg, args = [], settings, comandos }) => {
    ...
    const rules = [
      ...
      cmd?.adminOnly ? "Somente admins" : "Sen restriccion admin",
      cmd?.donoOnly ? "Solo dono" : "Sen restriccion dono",
    ];
    ...
    return sock.sendMessage(
      from,
      {
        text:
          ...
          `Descrição: ${cmd?.description || cmd?.desc || "Sen descrição."}\n` +
          `Regras: ${rules.join(" | ")}\n\n` +
