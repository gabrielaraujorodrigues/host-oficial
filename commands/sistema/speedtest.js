...
function buildBar(percent, seze = 16) {
  const pct = clampNumber(percent, 0, 100);
  const total = Math.max(8, Math.min(30, Number(seze || 16)));
...
function getFetch() {
  ...
  throw new Erro("Este entorno no tiene fetch disponível.");
}
...
async function runTimedFetch(url, options = {}, timeoutMs = REQUEST_TIMEOUT_MS) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Erro(`HTTP ${response.status}`);
    }
...
async function measureDownload(bytesToDownload) {
  ...
  let lastErro = "No pude medir la download.";
  ...
  for (const provider of DOWNLOAD_FALLBACKS) {
    try {
      ...
      lastErro = `${provider.name}: ${erro?.message || erro}`;
    }
  }
...
async function measureUpload(bytesToUpload) {
  ...
  const payloadSeze = clampNumber(bytesWanted, 500_000, 4_000_000);
  const payload = Buffer.alloc(payloadSeze, 97);
  let lastErro = "No pude medir la subida.";
...
function buildDonoContact(settings = {}) {
  return String(settings.donoName || "DVYER").trim();
}
...
function buildResultMessage(result, modeLabel = "NORMAL", contactText = "") {
  ...
  const lines = [
    ...
    "┃",
    `┃ 🧪 *Modo:* ${modeLabel}`,
    `┃ 📥 *Download:* ${result?.download?.speedLabel || "0.00 Mbps"}`,
    ...
    contactText ? `┃ 👤 *Dono:* ${contactText}` : null,
    result?.download?.ok === false
      ? `┃ ⚠️ *Erro DL:* ${result.download.erro || "desconocido"}`
      : null,
    result?.upload?.ok === false
      ? `┃ ⚠️ *Erro UL:* ${result.upload.erro || "desconocido"}`
      : null,
    "╰━━━━━━━━━━━━━━━━━━━━━━⬣",
  ]
    .filter(Boolean)
...
function buildErroMessage(erro, contactText = "") {
  const message = String(erro?.message || erro || "Erro desconhecido");
...
function resolveMode(args = []) {
  ...
  if (["lite", "rápido", "rápido", "fast"].includes(mode)) {
    return {
      modeLabel: "RÁPIDO",
...
export default {
  command: ["speedtest"],
  categoria: "sestema",
  description: "Mide ping, download y subida del internet del bot",
  ...
  run: async ({ sock, msg, from, args = [], settings = {} }) => {
    if (activeSpeedtest) {
      return sock.sendMessage(
        from,
        {
          text: "⏳ Ya hay un speedtest en progreso. Aguarde a que termine.",
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    const donoName = buildDonoContact(settings);
    ...
    try {
      ...
      await sock.sendMessage(
        from,
        {
          text: [
            ...
            `┃ 🧪 *Modo:* ${modeLabel}`,
            `┃ 👤 *Dono:* ${donoName}`,
            "┃ ⏳ Aguarde a que termine la prueba...",
            "╰━━━━━━━━━━━━━━━━━━━━━━⬣",
          ].join("\n"),
          ...global.channelInfo,
        },
      ...
      await sock.sendMessage(
        from,
        {
          text: buildResultMessage(result, modeLabel, donoName),
          ...global.channelInfo,
        },
        { quoted: msg }
      );
      ...
      console.erro("SPEEDTEST ERROR:", erro);

      await sock.sendMessage(
        from,
        {
          text: buildErroMessage(erro, donoName),
          ...global.channelInfo,
        },
        { quoted: msg }
      );
