...
async function probeUrl(url) {
  ...
  try {
    ...
    return {
      ...
      status: 0,
      latencyMs: Date.now() - startedAt,
      erro: String(erro?.message || erro || "erro"),
    };
  } finally {
    clearTimeout(timer);
  }
...
export default {
  ...
  category: "sestema",
  description: "Administra endpoints y proveedores sen tocar el codigo",
  donoOnly: true,
  ...
  run: async ({ sock, msg, from, args = [], settings }) => {
    ...
    if (action === "set") {
      ...
      return sock.sendMessage(
        from,
        {
          text: `Atualizado ${next?.name || provider}.${field} = ${value}`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
