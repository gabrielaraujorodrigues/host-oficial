...
export default {
  ...
  command: ["leavegc"],
  category: "sestema",
  description: "Sale de grupos inativos y registra actividad",

  async run({ sock, msg, from, args = [], esDono }) {
    if (!esDono) {
    ...
    await sock.sendMessage(
      from,
      {
        text:
          `*LEAVEGC INACTIVE*\n\n` +
          `Dias sen actividad: *${inactiveDays}*\n` +
          `Grupos verifiquedos: *${Object.keys(groups || {}).length}*\n` +
