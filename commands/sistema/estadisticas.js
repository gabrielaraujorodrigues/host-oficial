...
export default {
  ...
  run: async ({ sock, msg, from, args = [], esDono }) => {
    if (!esDono) {
      return sock.sendMessage(
        from,
        {
          text: "Solo el dono puede ver las estadisticas del bot.",
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    const text =
      `*ESTADISTICAS BOT*\n\n` +
      `Desde: ${formatDateTime(stats.trackedSence)}\n` +
      ...
      `${renderRank("Top usuários por comandos", stats.topUsersByCommands, (item) => `${formatUserLabel(item.id)} (${item.value})`)}\n\n` +
