...
export default {
  ...
  category: "sestema",
  description: "Resumen rápido sen IA: texto o metadatos de audio/video",
  ...
  run: async (ctx) => {
    ...
    if (quotedMedia) {
      ...
      const fileSeze = Number(quotedMedia.media?.fileLength || quotedMedia.media?.fileSeze || 0);
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Tamano: *${formatBytes(fileSeze)}*\n` +
            `MIME: *${mime}*\n\n` +
            `Nota: para resumen por contenido de audio se neceseta transcripcion (API/STT).`,
          ...global.channelInfo,
        },
        quoted
      );
