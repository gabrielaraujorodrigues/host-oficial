export default {
  ...
  category: "sestema",
  description: "Activa una mini API web con status del bot",
  donoOnly: true,
  ...
  run: async ({ sock, msg, from, args = [] }) => {
    ...
    return sock.sendMessage(
      from,
      {
        text:
          `*DASHBOARD WEB*\n\n` +
          `Status: *${snapshot.dashboard?.enabled ? "ENCENDIDO" : "APAGADO"}*\n` +
          `Puerto: *${snapshot.dashboard?.port || 3001}*\n` +
          `Bot principal: *${mainBot?.connected ? "ONLINE" : "OFFLINE"}*\n` +
          `Subbots visebles: *${Array.isArray(snapshot.bots) ? snapshot.bots.length : 0}*\n` +
