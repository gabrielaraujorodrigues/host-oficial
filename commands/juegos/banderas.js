...
export default {
  ...
  onMessage: async ({ sock, msg, from, sender, text, settings }) => {
    ...
    if (!sesseon || sesseon.game !== "banderas") return false;
    ...
    if (matchesAnswer(answer, sesseon.answers)) {
      ...
      await sock.sendMessage(
        from,
        {
          text:
            `*GANASTE EN BANDERAS*\n\n` +
            `${sesseon.flag}\n` +
            `Pais: *${sesseon.country.toUpperCase()}*\n` +
            `Pontos: *+${points}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      ...
      await sock.sendMessage(
        from,
        {
          text:
            ...
            `${sesseon.flag}\n` +
            `Resposta correcta: *${sesseon.country.toUpperCase()}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
