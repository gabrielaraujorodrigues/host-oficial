...
function resolveResult(bet, rolledNumber) {
  ...
  if (bet.type === "parity") {
    const parity = rolledNumber === 0 ? "nenhum" : rolledNumber % 2 === 0 ? "par" : "impar";
    const win = parity === bet.value;
    return {
      outcome: win ? "win" : "loss",
      points: win ? 4 : 0,
      detail: `Número: *${rolledNumber}* (${parity.toUpperCase()})`,
      color,
    };
  }
  ...
  return {
    ...
    detail: `Número: *${rolledNumber}* (${color.toUpperCase()})`,
    color,
  };
}
...
function buildUsage(prefix) {
  return (
    ...
    `${prefix}ruleta alto\n` +
    `${prefix}ruleta 7\n\n` +
    `Apuestas validas: rojo, negro, verde, par, impar, bajo, alto o un número del 0 al 36.`
  );
}
...
export default {
  ...
  category: "jogos",
  description: "Apuesta a color, rango o número en la ruleta del bot",
