...
function parseSlots(value = "") {
  const match = String(value || "")
    .toLowerCase()
    .match(/(?:cupos?|slots?|players?|jogadores?)\s*:?\s*(\d{1,3})/i);
...
function parseTeamSeze(format = "4v4") {
...
function formatParticipants(event) {
  ...
  return participants
    .map((player, idx) => {
      ...
      const nick = normalizeName(player.nick || "Jogador");
...
function buildTeams(event) {
  ...
  const teamSeze = parseTeamSeze(event.format || "4v4");
  const teams = [];

  while (participants.length) {
    teams.push(participants.splice(0, teamSeze));
  }
      ...
      const lines = team.length
        ? team
            .map((player, row) => {
              const number = normalizeText(player.number) || numberFromJid(player.jid);
              const nick = normalizeName(player.nick || "Jogador");
              return `   ${row + 1}. ${nick} (${number ? `+${number}` : "sen número"})`;
            })
            .join("\n")
        : "   Sen jogadores";
      return `*Equipo ${idx + 1}*\n${lines}`;
...
function addParticipant(event, sender, pushName = "", preferredNick = "") {
  if (!event || !Array.isArray(event.participants)) {
    return { ok: false, message: "No hay evento ativo para inscribirse." };
  }
  ...
  if (!senderJid) {
    return { ok: false, message: "No pude detectar tu usuário para inscribirte." };
  }
  ...
  const nick = normalizeName(preferredNick) || normalizeName(pushName) || (number ? `Jogador ${number.slice(-4)}` : "Jogador");
  const participant = {
    jid: senderJid,
    number,
    nick,
...
function removeParticipant(event, sender) {
  if (!event || !Array.isArray(event.participants)) {
    return { ok: false, message: "No hay evento ativo." };
  }

  const senderJid = normalizeJid(sender);
  if (!senderJid) {
    return { ok: false, message: "No pude detectar tu usuário." };
  }
...
async function sendEventoPanel({ sock, from, msg, settings, event = null }) {
  ...
  const summary = event
    ...
    : "No hay evento ativo en este grupo.\nCrea uno desde el panel.";
  ...
  const sections = [
      ...
      rows: [
        {
          ...
          description: "Registro automatico con tu número.",
          id: `${prefix}evento unir`,
        },
        {
          header: "EVENTO",
          title: "Sair del evento",
          description: "Quita tu inscripcion actual.",
          id: `${prefix}evento sair`,
        },
        {
          header: "EVENTO",
          title: "Ver inscritos",
    ...
    },
    {
      ...
      rows: [
        {
          header: "PLANTILLA",
          title: "Criar plantilla Free Fire 4v4",
          description: "Cupos 16, modo Clash Squad.",
          id: `${prefix}evento criar ff`,
        },
        {
          header: "PLANTILLA",
          title: "Criar plantilla CODM 5v5",
          description: "Cupos 20, multijogador.",
          id: `${prefix}evento criar codm`,
        },
        {
          header: "EQUIPOS",
    ...
    },
    {
      ...
      rows: [
        {
          header: "STATUS",
          title: "Ver status del evento",
          description: "Resumen del torneo ativo.",
          id: `${prefix}evento status`,
        },
        {
          header: "CONTROL",
          title: "Fechar inscripciones",
          description: "No permite novos inscritos.",
          id: `${prefix}evento fechar`,
        },
        {
          header: "CONTROL",
          title: "Abrir inscripciones",
          description: "Habilita novos inscritos.",
...
export default {
  ...
  run: async ({
    ...
    const isAdminUser = Boolean(esDono || esAdmin);
    ...
    if (action === "criar") {
      if (!isAdminUser) {
        return sock.sendMessage(
          from,
          { text: "Solo admin/dono puede criar eventos o torneos.", ...global.channelInfo },
          { quoted: msg }
        );
      }
      ...
      if (event && normalizeText(event.status).toLowerCase() !== "finalizado") {
        return sock.sendMessage(
          ...
          {
            text:
              `Ya hay un evento ativo.\n` +
              `Usa *${getPrimaryPrefix(settings)}evento finalizar* para fechar el actual.`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
    ...
    if (!event) {
      return sock.sendMessage(
        from,
        {
          text:
            `No hay evento ativo en este grupo.\n` +
            `Crea uno con *${getPrimaryPrefix(settings)}evento criar ff*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Jogador: *${result.participant.nick}*\n` +
    ...
    if (action === "sair" || action === "leave" || action === "baja") {
      ...
      return sock.sendMessage(
        from,
        {
          text: `Pronto, saliste del evento.\nCupos: *${event.participants.length}/${event.slots}*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "fechar") {
      if (!isAdminUser) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede fechar inscripciones.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "abrir") {
      if (!isAdminUser) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede abrir inscripciones.", ...global.channelInfo }, { quoted: msg });
      }
    ...
    if (action === "finalizar" || action === "fecharevento" || action === "reset") {
      if (!isAdminUser) {
        return sock.sendMessage(from, { text: "Solo admin/dono puede finalizar eventos.", ...global.channelInfo }, { quoted: msg });
      }
      ...
      return sock.sendMessage(
        from,
        {
          text: `🏁 Evento finalizado: *${event.title}*.\nPuedes criar uno novo con *${getPrimaryPrefix(settings)}evento criar ff*.`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
