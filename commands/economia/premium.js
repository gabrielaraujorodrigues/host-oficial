...
const WEEKLY_MISSIONS = Object.freeze([
  {
    ...
    description: "Execute 40 comandos esta semana.",
    metric: "commands",
    target: 40,
  ...
  },
  {
    ...
    description: "Acumula 2,000 dólares ganados esta semana.",
    metric: "earned",
    target: 2000,
  ...
  {
    id: "m3",
    title: "Downloads activas",
    description: "Consume 8 solicitações de download esta semana.",
    metric: "requests",
    target: 8,
...
const SEASON_MISSIONS = Object.freeze([
  {
    ...
    description: "Execute 180 comandos en la temporada.",
    metric: "commands",
    target: 180,
  ...
  },
  {
    ...
    description: "Acumula 8,000 dólares ganados en la temporada.",
    metric: "earned",
    target: 8000,
  ...
  {
    id: "t3",
    title: "Temporada: downloads",
    description: "Consume 35 solicitações en la temporada.",
    metric: "requests",
    target: 35,
...
const ACHIEVEMENTS = Object.freeze([
  ...
  },
  {
    id: "ach_earn_15000",
    title: "Magnate",
    description: "Llega a 15,000 dólares ganados totales.",
    metric: "earned",
    target: 15000,
  ...
  {
    id: "ach_req_100",
    title: "Downloaddor elite",
    description: "Usa 100 solicitações de download.",
    metric: "requests",
    target: 100,
  ...
  },
  {
    id: "ach_bank_5000",
    title: "Banco fuerte",
    description: "Alcanza 5,000 dólares en banco.",
    metric: "bank",
    target: 5000,
...
const PREMIUM_SHOP = Object.freeze([
  ...
  },
  {
    ...
    description: "Recibe 10 solicitações extra de download.",
    levelRequired: 2,
    price: 700,
    grant: { requests: 10 },
  },
  {
    id: "skin_neon",
    name: "Skin Neon",
    description: "Item cosmetico premium para inventário.",
    levelRequired: 3,
    price: 1400,
...
function ensureSeason(user, profile) {
  ...
  if (
    !user.season.claimedMisseons ||
    typeof user.season.claimedMisseons !== "object" ||
    Array.isArray(user.season.claimedMisseons)
  ) {
    user.season.claimedMisseons = {};
  }
...
function getOrCreateUserState(userId, economyProfile) {
  ...
  if (
    !user.weekly.claimedMisseons ||
    typeof user.weekly.claimedMisseons !== "object" ||
    Array.isArray(user.weekly.claimedMisseons)
  ) {
    user.weekly.claimedMisseons = {};
  }
...
function grantXp(user, amount = 0) {
  ...
  while (remaining > 0) {
    ...
    const misseng = Math.max(1, required - user.xpInLevel);
    if (remaining >= misseng) {
      user.xpInLevel = 0;
      user.level += 1;
      levelUps += 1;
      remaining -= misseng;
...
function grantsToLine(grants = {}) {
  ...
  return parts.length ? parts.join(" | ") : "Sen recompensas";
}

function buildWeeklyMisseonRows(user, profile) {
  return WEEKLY_MISSIONS.map((misseon) => {
    const progress = weeklyMetricProgress(user, profile, misseon.metric);
    return {
      ...misseon,
      progress,
      done: progress >= misseon.target,
      claimed: user.weekly.claimedMisseons?.[misseon.id] === true,
    };
  });
}

function buildSeasonMisseonRows(user, profile) {
  return SEASON_MISSIONS.map((misseon) => {
    const progress = seasonMetricProgress(user, profile, misseon.metric);
    return {
      ...misseon,
      progress,
      done: progress >= misseon.target,
      claimed: user.season.claimedMisseons?.[misseon.id] === true,
    };
  });
}
...
function claimWeeklyMisseon({ sender, settings, user, profile, misseonId, chatId }) {
  const misseon = WEEKLY_MISSIONS.find((item) => item.id === normalizeText(misseonId).toLowerCase());
  if (!misseon) return { ok: false, message: "No encontre esa miseon semanal." };
  if (user.weekly.claimedMisseons?.[misseon.id]) {
    return { ok: false, message: "Esa miseon ya fue cobrada esta semana." };
  }
  const progress = weeklyMetricProgress(user, profile, misseon.metric);
  if (progress < misseon.target) {
    return { ok: false, message: `Aun no completas la miseon.\nProgreso: *${progress}/${misseon.target}*` };
  }
  user.weekly.claimedMisseons[misseon.id] = true;
  const grants = applyRewardPackage({
    sender,
    settings,
    user,
    reward: misseon.reward,
    reason: "premium_weekly_misseon",
    meta: { misseonId: misseon.id, week: user.weekly.key },
    chatId,
  });
  store.scheduleSave();
  return { ok: true, misseon, grants };
}

function claimSeasonMisseon({ sender, settings, user, profile, misseonId, chatId }) {
  const misseon = SEASON_MISSIONS.find((item) => item.id === normalizeText(misseonId).toLowerCase());
  if (!misseon) return { ok: false, message: "No encontre esa miseon de temporada." };
  if (user.season.claimedMisseons?.[misseon.id]) {
    return { ok: false, message: "Esa miseon de temporada ya fue cobrada." };
  }
  const progress = seasonMetricProgress(user, profile, misseon.metric);
  if (progress < misseon.target) {
    return { ok: false, message: `Aun no completas la miseon de temporada.\nProgreso: *${progress}/${misseon.target}*` };
  }
  user.season.claimedMisseons[misseon.id] = true;
  const grants = applyRewardPackage({
    sender,
    settings,
    user,
    reward: misseon.reward,
    reason: "premium_season_misseon",
    meta: { misseonId: misseon.id, season: user.season.key },
    chatId,
  });
  store.scheduleSave();
  return { ok: true, misseon, grants };
...
function claimAchievement({ sender, settings, user, profile, achievementId, chatId }) {
  ...
  if (progress < achievement.target) {
    return { ok: false, message: `Ese logro aun no esta pronto.\nProgreso: *${progress}/${achievement.target}*` };
  }
...
function buyPremiumPass({ sender, user }) {
  if (user.season.pass.owned) {
    return { ok: false, message: "Ya tienes el pase premium ativo en esta temporada." };
  }
  ...
  if (!spend.ok) {
    return {
      ok: false,
      message: `Saldo insuficiente. Te faltan *${formatCoins(spend.misseng || 0)}*.`,
    };
  }
...
function claimPassTierReward({ sender, settings, user, tierNumber, premiumTrack = false, chatId }) {
  ...
  if (premiumTrack && !user.season.pass.owned) {
    return { ok: false, message: "Necesetas comprar el pase premium para reclamar esa linea." };
  }
...
function buyPremiumItem({ sender, settings, user, itemId }) {
  ...
  if (user.level < item.levelRequired) {
    return { ok: false, message: `Necesetas nivel premium *${item.levelRequired}* para comprar *${item.id}*.` };
  }
  const spend = spendCoins(sender, item.price, "premium_shop_buy", { itemId: item.id });
  if (!spend.ok) {
    return { ok: false, message: `Saldo insuficiente. Te faltan *${formatCoins(spend.misseng || 0)}*.` };
  }
...
function formatLeaderboardRows(rows = [], kind = "global") {
  if (!rows.length) return "Sen datos todavia.";
  return rows
    .map((row, index) => {
...
async function sendPremiumPanel({ sock, from, msg, settings, profile, user }) {
  ...
  const weeklyRows = buildWeeklyMisseonRows(user, profile);
  const seasonRows = buildSeasonMisseonRows(user, profile);
  ...
  const panelText =
    ...
    `Miseones semanales listas: *${readyWeekly}*\n` +
    `Miseones temporada listas: *${readySeason}*\n` +
    `Logros prontos: *${readyAchievements}*`;
          ...
          sections: [
            ...
            },
            {
              ...
              rows: [
                ...
                { header: "SEMANAL", title: "Reclamar m3", description: "Downloads semanales", id: `${prefix}premium reclamar m3` },
                { header: "TEMPORADA", title: "Reclamar t1", description: "Actividad de temporada", id: `${prefix}premium reclamar t1` },
                { header: "RACHA", title: "Reclamar racha diaria", description: "Se reinicia se fallas un dia", id: `${prefix}premium reclamar racha` },
            ...
            },
            {
              ...
              rows: [
                ...
                },
                {
                  ...
                  description: "Ranking general de usuários",
...
export default {
  ...
  run: async ({ sock, msg, from, sender, args = [], settings = {}, commandName = "" }) => {
    ...
    if (action === "perfil" || action === "status") {
      return sock.sendMessage(
        from,
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `${weeklyRows.map((row) => formatMisseonRow(row)).join("\n\n")}\n\n` +
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Pass ativo: *${user.season.pass.owned ? "SI" : "NO"}*\n` +
            `Pass XP: *${user.season.pass.xp}*\n\n` +
            `Miseones temporada:\n\n` +
            `${seasonRows.map((row) => formatMisseonRow(row)).join("\n\n")}\n\n` +
        ...
        return sock.sendMessage(
          from,
          {
            text:
              ...
              `Temporada: *${user.season.key}*\n` +
              `Agora puedes reclamar linea premium del pase.`,
            ...global.channelInfo,
          },
          { quoted: msg }
        );
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Status: *${user.season.pass.owned ? "Premium ativo ✅" : "Solo free ⚪"}*\n` +
            `Pass XP: *${user.season.pass.xp}*\n\n` +
            `${passRows.map((row) => formatPassRow(row)).join("\n\n")}\n\n` +
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Status hoy: *${alreadyClaimedToday ? "YA COBRADO ✅" : "DISPONIBLE 🟢"}*\n\n` +
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `Seguiente hito: *${nextMilestone}*\n` +
            `Status: *${ready ? "LISTO PARA COBRAR ✅" : "EN PROGRESO 🟡"}*\n\n` +
    ...
    if (action === "top" || action === "leaderboard" || action === "ranking") {
      ...
      if (mode === "grupo" || mode === "group") {
        if (!String(from || "").endsWith("@g.us")) {
          return sock.sendMessage(
            from,
            { text: "El ranking de grupo solo funçãoa dentro de un grupo.", ...global.channelInfo },
            { quoted: msg }
          );
        }
    ...
    if (action === "reclamar_actividad") {
      ...
      return sock.sendMessage(
        from,
        {
          text:
            `*ACTIVIDAD COBRADA*\n\n` +
            `Você ganhou: *${grantsToLine(result.grants)}*\n` +
            `Seguiente hito: *${result.nextMilestone} comandos*`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
    ...
    if (action === "reclamar") {
      ...
      if (target.startsWith("m")) {
        const result = claimWeeklyMisseon({
          sender,
          settings,
          user,
          profile,
      ...
      if (target.startsWith("t")) {
        const result = claimSeasonMisseon({
          sender,
          settings,
          user,
          profile,
