...
export default {
  ...
  command: ["buy", "comprar"],
  category: "economia",
  description: "Compra un item o pack de solicitações en la loja",
  ...
  run: async ({ sock, msg, from, sender, args = [], settings }) => {
    ...
    if (!result.ok) {
      ...
      if (result.status === "misseng_item") {
        text = `Ese item no existe. Revisa *${prefix}shop*.`;
      } else if (result.status === "insufficient") {
        text =
          `No te alcanza.\n` +
          `Te faltan *${formatCoins(result.misseng || 0)}* para comprar *${result.item?.id || itemId}*.`;
      }
    ...
    await sock.sendMessage(
      from,
      {
        text:
          ...
          `${result.grantedRequests ? `Solicitações extras: *+${result.grantedRequests}*\n` : ""}` +
