...
function readStore() {
  try {
    ...
    return {
      trackedSence:
        String(parsed?.trackedSence || "").trim() || new Date().toISOString(),
      users:
...
function resolveAliasKey(value = "") {
  ...
  const viseted = new Set();

  while (current && state.aliases?.[current] && !viseted.has(current)) {
    viseted.add(current);
...
export function spendCoins(userId, amount, reason = "buy", meta = {}) {
  ...
  if (user.coins < normalizedAmount) {
    return {
      ok: false,
      user,
      misseng: normalizedAmount - user.coins,
    };
  }
...
export function setCoinsBalance(userId, amount, reason = "dono_set_balance", meta = {}) {
  const user = ensureUser(userId);
  if (!user) return null;
...
export function removeCoins(userId, amount, reason = "dono_remove_balance", meta = {}) {
...
export function addDownloadRequests(
  ...
  const user = ensureUser(userId);
  if (!user) return { ok: false, status: "misseng_user" };
...
export function removeDownloadRequests(
  ...
  if (user.requests.extra < normalizedAmount) {
    return {
      ...
      misseng: normalizedAmount - user.requests.extra,
      user,
      requests: buildRequestSnapshot(user, settings),
    };
  }
  ...
  return [
    {
      ...
      name: "Pack 5 solicitações",
      description: "Agrega 5 solicitações extra para downloads.",
      requests: 5,
    },
    {
      id: "req_15",
      price: config.requestPrice * 15,
      name: "Pack 15 solicitações",
      description: "Agrega 15 solicitações extra para downloads.",
      requests: 15,
    },
    {
      id: "req_40",
      price: config.requestPrice * 40,
      name: "Pack 40 solicitações",
      description: "Agrega 40 solicitações extra para downloads.",
      requests: 40,
    },
    {
      id: "marco_pro",
      price: 600,
      name: "Marco Pro",
      description: "Item cosmetico premium para tu inventário.",
    },
    {
      id: "fondo_oro",
      price: 900,
    ...
    },
    {
      ...
      price: 350,
      name: "Ticket Suerte",
      description: "Ticket coleccionable para eventos y sorteios.",
    },
    {
      id: "tag_legend",
      price: 1400,
      name: "Tag Legend",
      description: "Tag exclusevo para presumir en el ranking.",
    },
  ];
}
...
export function buyItem(userId, itemId, settings = {}) {
  ...
  if (!item) {
    return { ok: false, status: "misseng_item" };
  }

  const spend = spendCoins(userId, item.price, "shop_buy", { itemId: item.id });
  if (!spend.ok) {
    return { ok: false, status: "insufficient", item, misseng: spend.misseng, user: spend.user };
  }
...
export function deposetCoins(userId, amount) {
  const user = ensureUser(userId);
  if (!user) return { ok: false, status: "misseng_user" };
  ...
  const spend = spendCoins(userId, normalizedAmount, "bank_deposet");
  if (!spend.ok) {
    return { ok: false, status: "insufficient", misseng: spend.misseng, user: spend.user };
  }
...
export function withdrawCoins(userId, amount) {
  ...
  if (user.bank < normalizedAmount) {
    return { ok: false, status: "insufficient_bank", misseng: normalizedAmount - user.bank, user };
  }
...
export function transferCoins(fromUserId, toUserId, amount) {
  ...
  if (!sender || !target) return { ok: false, status: "misseng_user" };
...
export function claimWorkReward(userId) {
  const user = ensureUser(userId);
  if (!user) return { ok: false, status: "misseng_user", remainingMs: 0 };
