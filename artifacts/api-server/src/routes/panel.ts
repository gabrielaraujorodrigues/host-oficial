...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  header .badge{
    ...
    padding:3px 8px;border-radius:12px;letter-spacing:.5px;margin-left:auto;
  }
  ...
  .sending-row{display:flex;gap:10px;margin-top:8px}
  .sending-row input{flex:1}
</style>
</head>
<body>
<header>
...
async function loadStatus(){
  try{
    ...
    badge.textContent = d.status === 'conectado' ? '● Conectado' : d.status === 'desconectado' ? '● Desconectado' : '● Desconhecido';
...
async function loadSettings(){
  ...
  }catch(e){showToast('Erro ao carregar config',true);}
}
